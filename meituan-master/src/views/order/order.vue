    ...
    <article v-else ref="orderWrapper">
      <div class="container">
        <router-link v-for="item in ordersList" :key="item.id" tag="section" :to="'/order_detail?id='+ item.id">
          ...
          <div class="price-container"><span>共{{item.foods.length}}件商品，总额</span><span>￥{{item.total_price}}</span></div>
          <div class="footer">
          <!--
            <button class="paynotice" @click="payNotice(item.id)">订单已收到</button>
          -->
...
<script>
  import {orders,payNotice} from '@/api/order'
  ...
  export default {
    ...
    methods:{
      payNotice(outTradeNo){
        payNotice({outTradeNo:outTradeNo})
      }
    },
    mounted() {
