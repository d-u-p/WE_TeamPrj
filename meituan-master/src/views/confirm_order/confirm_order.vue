    ...
    methods: {
      submit() {
        ...
        if (this.preventRepeat)
          return;
        this.alertText = '订单提交成功，准备跳转';
              // this.showTip = true;
              // setTimeout(()=>{
              //   this.$router.push({path: '/order_detail', query: {id: this.orderData.order_id}})
              // },1000);
        ...
        submitOrder({restaurant_id: this.restaurant_id,foods, address_id: this.defineAddress.id}).then((response) => {
          if (response.data.status === 200) {
            this.$router.push({path: '/order_detail', query: {id: response.data.order_id}})
